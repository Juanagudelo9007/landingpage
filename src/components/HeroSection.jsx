...
const HeroSection = () => {
  ...
  const opacity = useTransform(scrollYProgress, [0, 0.90], [1, 0]);
  const scale = useTransform(scrollYProgress, [0, 0.9], [1, 0]);
  const y = useTransform(scrollY, [0, 700], [0, -550]);
  ...
  return (
    <motion.div
      ...
      className="h-screen w-full flex items-center justify-center px-6 text-2xl"
    >
      <div className="flex flex-col md:flex-row items-center gap-8">
        ...
        <motion.img 
        style={{y}}
        src={img1} alt="Hero" className="w-80 h-auto rounded" />
      </div>
    </motion.div>
  );
};
